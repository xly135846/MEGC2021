...
if __name__ == '__main__':
    ...
    parser.add_argument('--path_save', type=str, default="./output/preprocess_npy", 
                                   help="tthe path of saving the data after preprocess", metavar='N')
    ...
    print("Micro num:", len(label_micro), len(path_micro))
    print(path_micro)
    ...
    for file in range(len(path_micro)):
        ...
        if os.path.exists(save_path+"/"+str(path_micro[file].split("/")[-1][:-1])+"_"+Mode+".npy"):
            print(str(path_micro[file].split("/")[-1][:-1])+"_micro.npy", "is exists")
        else:
            all_jpgs = glob.glob(path+"longVideoFaceCropped/"+path_micro[file]+"/*.jpg")
            print(path_micro[file], label_micro[file], len(all_jpgs))
            ...
            print('fd:', fd, "fll:", len(fll))

            process_img = []
            for i in range( int(len(all_jpgs)/4)-1):

                img_path_1 = all_jpgs[i*4]
                img_path_2 = all_jpgs[(i+1)*4]
                ...
                process_img.append(final_image)
                test_arr = normal_arr(final_image)
